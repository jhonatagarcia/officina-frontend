...
import {
  PlateChip,
  VehicleIdentityCell,
} from '@/components/shared/table-identity-cells';
...
const toneMap: Record<
  MetricTone,
  { text: string; soft: string; line: string; fill: string; bar: string }
> = {
  orange: {
    text: 'text-primary',
...
const alertVariantMap: Record<
  DashboardOperationalAlert['severity'],
  'danger' | 'warning' | 'info'
> = {
  danger: 'danger',
  warning: 'warning',
          ...
          {typeof delta === 'number' ? (
            <span
              className={cn(
                ...
                toneMap[deltaTone].soft,
                toneMap[deltaTone].text,
              )}
            >
              {delta >= 0 ? (
          ...
          {animatedValues.map((item, index) => (
            <div key={index} className="flex h-full flex-col justify-end gap-3">
              ...
              <div
                ...
                style={{
                  height:
                    item > 0 ? `${Math.max(18, (item / maxValue) * 78)}%` : 0,
                }}
              />
            </div>
          ))}
        ...
        <div className="mt-4 grid grid-cols-6 px-6 text-center text-sm font-semibold text-muted-foreground/70">
          {months.map((month, index) => (
            <span
              key={month}
              className={
                index === months.length - 1 ? 'text-primary' : undefined
              }
            >
              {month}
            </span>
...
function StatusDonutCard({
  ...
  const inProgressEnd =
    readyEnd + (animatedInProgress / animatedSafeTotal) * 100;
  const pendingEnd =
...
function DashboardTable({ children }: { children: ReactNode }) {
  return (
    <div className="overflow-x-auto">
      <Table className="min-w-[860px] text-base">{children}</Table>
    </div>
  );
}
...
function DashboardTableHead({
  ...
  return (
    <TableHead
      className={cn(
        'h-14 px-6 text-[13px] font-bold tracking-[0.1em] text-muted-foreground',
        className,
      )}
    >
      {children}
...
function DashboardTableCell({
  ...
  return (
    <TableCell className={cn('px-6 py-5 text-base', className)}>
      {children}
    </TableCell>
  );
}
...
export function DashboardPage() {
  ...
  const activeServiceOrdersTotalPages = Math.max(
    1,
    Math.ceil(
      (query.data?.activeServiceOrders.length ?? 0) /
        ACTIVE_SERVICE_ORDERS_PAGE_SIZE,
    ),
  );
  const pendingBudgetsTotalPages = Math.max(
    1,
    Math.ceil(
      (query.data?.pendingBudgets.length ?? 0) /
        DASHBOARD_SECONDARY_TABLE_PAGE_SIZE,
    ),
  );
  const inventory = query.data?.inventory;
  const { sortedItems, sortState, requestSort } = useSortableData(
  ...
  const lowStockItemsTotalPages = Math.max(
    1,
    Math.ceil(sortedItems.length / DASHBOARD_SECONDARY_TABLE_PAGE_SIZE),
  );

  useEffect(() => {
    setActiveServiceOrdersPage((page) =>
      Math.min(page, activeServiceOrdersTotalPages),
    );
    ...
    pendingBudgetsTotalPages,
    lowStockItemsTotalPages,
  ]);
  ...
  const currentActiveServiceOrdersPage = Math.min(
    activeServiceOrdersPage,
    activeServiceOrdersTotalPages,
  );
  const activeServiceOrdersStartIndex =
    (currentActiveServiceOrdersPage - 1) * ACTIVE_SERVICE_ORDERS_PAGE_SIZE;
  const paginatedActiveServiceOrders = activeServiceOrders.slice(
    activeServiceOrdersStartIndex,
    activeServiceOrdersStartIndex + ACTIVE_SERVICE_ORDERS_PAGE_SIZE,
  );
  const currentPendingBudgetsPage = Math.min(
    pendingBudgetsPage,
    pendingBudgetsTotalPages,
  );
  const pendingBudgetsStartIndex =
    (currentPendingBudgetsPage - 1) * DASHBOARD_SECONDARY_TABLE_PAGE_SIZE;
  const paginatedPendingBudgets = pendingBudgets.slice(
    pendingBudgetsStartIndex,
    pendingBudgetsStartIndex + DASHBOARD_SECONDARY_TABLE_PAGE_SIZE,
  );
  const currentLowStockItemsPage = Math.min(
    lowStockItemsPage,
    lowStockItemsTotalPages,
  );
  const lowStockItemsStartIndex =
    (currentLowStockItemsPage - 1) * DASHBOARD_SECONDARY_TABLE_PAGE_SIZE;
  const paginatedLowStockItems = sortedItems.slice(
    lowStockItemsStartIndex,
    lowStockItemsStartIndex + DASHBOARD_SECONDARY_TABLE_PAGE_SIZE,
  );
  ...
  const activeOrderTotal =
    openOrdersCount + inProgressOrdersCount + readyOrdersCount;
  const projectedBalance = financial.netBalanceValue;
  const averageTicket = financial.averageTicket;
  const periodSubtitle = dashboardPeriodLabels[dashboardPeriod];
  const stockUnitsAtRisk = inventoryData.lowStockItems.reduce(
    (total, item) => total + Math.max(item.minimumQuantity - item.quantity, 0),
    0,
  );
  const sparklineFromValue = (value: number) => {
    const base = Math.max(value, 1);
    return [0.62, 0.74, 0.69, 0.83, 0.94, 1].map((factor) =>
      Math.round(base * factor),
    );
  };
  const orderMonthSeries = [0.66, 0.76, 0.8, 0.84, 0.93, 1].map((factor) =>
    Math.round(activeOrderTotal * factor),
  );
  const clientGroups = [...activeServiceOrders, ...pendingBudgets].reduce<
    Record<string, { count: number; total: number }>
  >((groups, item) => {
    const clientName =
      'clientName' in item
        ? item.clientName
        : (item.client?.name ?? 'Cliente sem nome');
  ...
  const topClients = Object.entries(clientGroups)
    .map(([label, group]) => ({
      label,
      detail: `${group.count} OS/orçamento(s)`,
      value: formatCurrency(group.total),
    }))
    .sort(
      (left, right) =>
        Number(right.value.replace(/\D/g, '')) -
        Number(left.value.replace(/\D/g, '')),
    )
    .slice(0, 5);
  const mechanicGroups = activeServiceOrders.reduce<Record<string, number>>(
    (groups, order) => {
  ...
  const topMechanics = Object.entries(mechanicGroups)
    .map(([label, count]) => ({
      label,
      detail: `${count} OS em fluxo`,
      value: `${count} OS`,
    }))
    .sort(
      (left, right) =>
        Number.parseInt(right.value, 10) - Number.parseInt(left.value, 10),
    )
    .slice(0, 5);
  const partGroups = activeServiceOrders.reduce<
    Record<string, { quantity: number; detail: string }>
  >((groups, order) => {
  ...
  const bestSellingParts = Object.entries(partGroups)
    .map(([label, group]) => ({
      label,
      detail: group.detail,
      value: `${group.quantity} un`,
    }))
    .sort(
      (left, right) =>
        Number.parseFloat(right.value) - Number.parseFloat(left.value),
    )
    .slice(0, 5);
  ...
  const activeMechanicsCount = workforce.activeEmployees;
  ...
  const getAlertAction = (alert: DashboardOperationalAlert) => {
    ...
    if (alert.id === 'pending-budgets') {
      return {
        ...
        onClick: () =>
          scrollToDashboardTable(
            'dashboard-pending-budgets-table',
  ...
  const dashboardWidgets = [
    ...
    },
    {
      ...
      render: () => (
        <MetricCard
          title="Pagamentos a receber"
          value={financial.pendingServiceOrderPaymentsValue}
          formatter={formatCurrency}
          delta={19.8}
          tone="green"
          icon={Receipt}
    ...
    },
    {
      ...
      render: () => (
        <MetricCard
          ...
          tone="orange"
          icon={ClipboardList}
          sparkline={sparklineFromValue(readyOrdersCount)}
        />
      ),
    },
    {
    ...
    },
    {
      ...
      render: () => (
        <MetricCard
          title="Tempo médio de execução"
          value={operational.averageExecutionDays}
          formatter={formatWithSuffix('dias')}
          delta={-7.4}
          tone="green"
          icon={Gauge}
        />
    ...
    },
    {
      ...
      render: () => (
        <MetricCard
          ...
          tone="orange"
          icon={Users}
          sparkline={sparklineFromValue(clients.new)}
        />
      ),
    },
    {
    ...
    },
    {
      ...
      render: () => (
        <MetricCard
          ...
          formatter={formatWithSuffix('itens')}
          delta={20}
          tone="amber"
          icon={AlertTriangle}
        />
    ...
    },
    {
      ...
      render: () => (
        <MetricCard
          title="Reposição necessária"
          value={stockUnitsAtRisk}
          formatter={formatWithSuffix('un')}
          delta={16.7}
          tone="orange"
          icon={PackageSearch}
          sparkline={sparklineFromValue(stockUnitsAtRisk)}
        />
      ),
    },
    {
      ...
      render: () => (
        <MetricCard
          ...
          tone="orange"
          icon={PackageCheck}
          sparkline={sparklineFromValue(inventoryData.lowStockCount)}
        />
      ),
    },
    {
        ...
        <RankingCard
          ...
          items={
            topClients.length
              ...
              : [
                  {
                    label: 'Sem clientes no período',
                    detail: 'Aguardando movimentação',
                    value: '-',
                  },
                ]
          }
    ...
    },
    {
      ...
      render: () => (
        <RankingCard
          title="Peças mais vendidas"
          items={bestSellingParts.length ? bestSellingParts : fallbackParts}
        />
      ),
    },
    {
        ...
        <RankingCard
          ...
          items={
            topMechanics.length
              ...
              : [
                  {
                    label: 'Sem mecânico atribuído',
                    detail: 'Aguardando distribuição',
                    value: '-',
                  },
                ]
          }
                ...
                <TableBody>
                  {paginatedLowStockItems.map((item) => (
                    <TableRow key={item.id} className="hover:bg-muted/40">
                      ...
                      <DashboardTableCell className="font-bold [font-variant-numeric:tabular-nums]">
                        {item.quantity}
                      </DashboardTableCell>
                      <DashboardTableCell className="text-muted-foreground [font-variant-numeric:tabular-nums]">
              ...
              <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/10 p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    ...
                    <p>
                      Fluxo operacional está estável, sem pendências
  ...
  return (
    <PageContainer>
      <PageHeader
        title="Dashboard"
        description="Visão operacional do negócio em tempo real."
      >
        <Button

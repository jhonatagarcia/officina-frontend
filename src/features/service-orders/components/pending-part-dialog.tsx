...
export function PendingPartDialog({
  ...
  const selectedItem = inventoryOptionsQuery.data?.find(
    (item) => item.value === inventoryItemId,
  );
  const availableQuantity =
    selectedItem?.quantity ?? pendingPart?.quantityAvailable ?? 0;
  const selectedItemOutOfStock =
    Boolean(selectedItem) && availableQuantity <= 0;
  const canSubmit =
    Boolean(inventoryItemId) && quantityRequired > 0 && !isSubmitting;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-xl rounded-2xl">
        <DialogHeader>
          <DialogTitle>
            {pendingPart ? 'Editar peça pendente' : 'Adicionar peça pendente'}
          </DialogTitle>
          <DialogDescription>
            Informe apenas a peça que está segurando esta OS. Você pode ajustar
            ou cancelar depois.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid gap-4 md:grid-cols-[1fr_160px]">
          <div className="space-y-2">
            <Label htmlFor="pending-part-inventory">Peça do estoque</Label>
            <Select
              disabled={inventoryOptionsQuery.isLoading || isSubmitting}
              value={inventoryItemId}
              onValueChange={setInventoryItemId}
            >
              <SelectTrigger id="pending-part-inventory">
                <SelectValue
                  placeholder={
                    inventoryOptionsQuery.isLoading
                      ? 'Carregando peças...'
                      : 'Selecione a peça'
                  }
                />
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(event) =>
                setQuantityRequired(Number(event.target.value))
              }
            />
          </div>
          ...
          <div className="rounded-xl border border-border-soft bg-stone-50 p-4 text-sm md:col-span-2">
            <p className="font-semibold text-muted-foreground">
              ...
              <span className="text-lg font-bold text-foreground dark:text-slate-950">
                {availableQuantity}
              </span>
            </p>
            {selectedItemOutOfStock ? (
              <p className="mt-3 font-semibold text-red-600 dark:text-red-400">
                Produto não disponível em estoque. Será necessário efetuar a
                compra do produto.
              </p>
            ) : null}
          </div>
          ...
          <div className="space-y-2 md:col-span-2">
            <Label htmlFor="pending-part-arrival">
              Previsão de chegada opcional
            </Label>
            <Input
